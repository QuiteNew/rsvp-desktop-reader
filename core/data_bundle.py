...
brief. gui/components/settings_window.py collects the file path and the
user's Replace/Expand/Cancel choice; gui/app.py calls the functions
here, then closes the app -- an import always requires a restart, since
the rest of the running UI (sidebar list, space selector, current
transcript, window layout) has no live-reload path back to a bundle's
contents.
"""
