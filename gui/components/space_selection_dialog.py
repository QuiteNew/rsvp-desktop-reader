...
class SpaceSelectionDialog(ctk.CTkToplevel):
    ...
    whole dialog (see gui/app.py's _handle_space_delete_confirmed()), since
    ...
    def __init__(
        ...
        # than just update_idletasks(): this window uses
        # CTkScrollableFrame, whose internal scroll-region sizing is
        # driven by real <Configure> events, which only update()
        # dispatches.
    ...
    def _add_space_row(self, space_name: str, is_current: bool) -> None:
        ...
        plus a rename and delete icon that apply regardless of
        ...
        whole problem the same way
        gui/components/transcript_list_body.py's add_entry() already
        does, one call per entry, which is also why none of the lambdas
        below need a default-arg capture trick for on_select: each one
        already closes over a fresh, per-call local."""
        ...
        if is_current:
            # A plain label, not a button: full color, not clickable, and
            # can't pick up any washed-out "disabled" button styling.
