...
class Divider(ctk.CTkFrame):
    ...
    practice, so this keeps the same thin visual line but gives the mouse
    ...
    began via on_drag, measured from the original press point every time,
    not incrementally, so the caller can always compute a fresh target
    size as (size_at_drag_start + delta) without drift building up.
    """
